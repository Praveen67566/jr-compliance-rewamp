...
The completed scope is the new home page, including its shared header and
footer. It intentionally does **not** migrate the other legacy routes yet.
...
| Path | Role |
| --- | --- |
| `site/` | 354-page legacy Webflow archive; `site/index.html` is the only home-page content source used so far. |
| `frontend/` | New Next.js 16 App Router application. This is the active frontend. |
| `frontend/data/homepage-fallback.ts` | Design-stage fallback content extracted and normalized from legacy `index.html`. It makes the page work before Strapi is deployed. |
| `frontend/lib/types.ts` | Rendering contract (`HomepageContent`) shared by fallback data and the Strapi adapter. |
| `frontend/lib/strapi.ts` | Server-only Strapi v5 REST client, explicit populate paths, media URL handling, and schema-to-UI mapper. |
| `frontend/components/` | Reusable shell components; `home/` contains interactive hero, service tabs, FAQ, and page composition. |
...
- A modern, animated, mobile-responsive home page at `/`.
...
first validating the destination. The new home-only build currently uses
on-page anchors intentionally; later route migration should replace these with
validated CMS-managed links.

## Strapi integration behavior

1. With no `STRAPI_URL`, the app renders `homepage-fallback.ts`.
2. With `STRAPI_URL` and a server-only `STRAPI_API_TOKEN`, the app requests the
   published `home-page` and `site-setting` single types every 60 seconds.
3. `frontend/lib/strapi.ts` explicitly populates only the nested relations and
   media the homepage requires. Do not replace this with `populate=deep`.
4. The adapter maps the documented Strapi v5 fields to `HomepageContent`; it
...
Strapi v5 TypeScript app **inside `cms/`**, then build the exact single types,
components, collection types, roles, media policy, and seeding order in
...
3. Add a real enquiry/contact integration only after the target receiver,
   validation, consent copy, and spam protection are explicitly decided.
4. Add deploy configuration and a Strapi publish webhook to revalidate the
   `jr-homepage` / `jr-site-settings` cache tags after publishing.
